...
\def\petal{
  ...
  .. controls (0.05,0.05) and (0.77,0.77) .. (0,1.15) %
  .. controls (-0.77,0.77) and (-0.05,0.05) .. (0,0) %
}
...
% Now draw an inner circle
%\draw [black, fill=white] (0,0) circle(0.60cm);

%foreach \angle in {\petalangle,\petalstephint,...,360} {
%    \pgfmathparse{\angle+3*\petalangle/4}\edef\thisangle{\pgfmathresult};
%    \draw (0:0) -- (\thisangle:0.66);
%}

\foreach \angle in {\petalangle,\petalstephint,...,360} {
    \pgfmathparse{\angle+\petalangle/4}\edef\thisangle{\pgfmathresult};
    \draw (0:0) -- (\thisangle:0.82);
}

\draw [black, fill=white] (0,0) circle(0.6cm);
\draw [black, fill=white] (0,0) circle(0.5cm);


\draw [black, fill=white] (0,0) circle(0.25cm);
